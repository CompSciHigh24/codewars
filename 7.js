...
app.get("/", (request, response) => {
    response.json("<h1>Welcome to the Animal Shelter Network </h1>")
})

app.get("/api/cat", (request, response) => {
    response.send(animalShelterData.cats)
})

app.get("/api/shelters", (request, response) => {
    response.send(animalShelterData.shelters)
})

app.get("/docs", (request, response) => {
    response.json("Go to /api/cat to see cats for adoption and /api/shelters to see shelters in the area")
})

app.get("/adopt/cat", (request, response) => {
    response.json("A cat you can adopt is " + animalShelterData.cats[0].name)
})
